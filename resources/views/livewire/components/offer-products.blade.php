<div>
    <!-- Header Section -->
    <div class="bg-gradient-to-r from-red-500 to-orange-500 rounded-lg shadow-sm p-6 mb-6 text-white">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold mb-2">🔥 Ofertas Especiais</h2>
                <p class="text-red-100">Produtos com os melhores descontos - tempo limitado!</p>
            </div>
            <div class="text-right">
                <div class="text-3xl font-bold">ATÉ 70% OFF</div>
                <div class="text-sm text-red-100">Economize mais</div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
        <div class="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
            <div class="flex flex-wrap gap-4 flex-1">
                ...
                <div class="min-w-[200px]">
                    <input type="text" 
                           ...
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500 text-sm">
                </div>
            ...
            <div class="flex items-center gap-4">
                <span class="text-sm text-gray-600">{{ $products ? $products->count() : 0 }} oferta(s)</span>
            ...
            <div class="bg-white rounded-lg shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden group">
                ...
                <div class="aspect-square bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center relative overflow-hidden group">
                    ...
                    @if($product->sale_price && $product->sale_price > 0)
                        ...
                        <div class="absolute top-2 left-2">
                            <span class="bg-gradient-to-r from-red-500 to-pink-500 text-white text-xs px-2 py-1 rounded-full font-bold animate-pulse">
                                -{{ $discount }}% OFF
                            </span>
                        </div>
                    @endif
                    ...
                    <div class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-30 transition-all duration-300 flex items-center justify-center space-x-3">
                        ...
                        <button wire:click="viewProduct({{ $product->id }})" 
                                class="bg-white text-gray-700 p-2 rounded-full shadow-lg transform scale-0 group-hover:scale-100 transition-all duration-300 hover:bg-red-500 hover:text-white">
                ...
                <div class="p-3">
                    ...
                    <div class="flex items-center mb-2">
                        ...
                        <span class="ml-1 text-xs text-gray-500">({{ $product->rating_count ?? 0 }})</span>
                    </div>
                    ...
                    <div class="mb-3">
                        <div class="flex flex-col space-y-1">
                            ...
                            <span class="text-lg font-bold text-red-600">
                                {{ number_format($product->sale_price, 0, ',', '.') }} Kz
                            </span>
                            <!-- Original Price -->
                    ...
                    <div class="space-y-2">
                        <button wire:click="addToCart({{ $product->id }})" 
                                class="w-full bg-gradient-to-r from-red-500 to-pink-500 text-white py-2 px-3 rounded-lg text-xs font-medium hover:from-red-600 hover:to-pink-600 transition-colors">
                        ...
                        <button wire:click="viewProduct({{ $product->id }})" 
                                class="w-full text-red-500 border border-red-500 py-1.5 px-3 rounded-lg text-xs font-medium hover:bg-red-50 transition-colors">
                            Ver Detalhes
                        </button>
                    </div>
                </div>
                                        ...
                                        @foreach($currentImages as $index => $image)
                                            <button wire:click="setMainImage({{ $index }})" 
                                                    class="flex-shrink-0 w-16 h-16 bg-gray-100 rounded-lg overflow-hidden border-2 transition-colors
                                                           {{ $index === $mainImageIndex ? 'border-red-500' : 'border-gray-200 hover:border-gray-300' }}">
                            ...
                            <div class="w-full sm:w-1/2">
                                ...
                                <div class="mb-2">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">
                                        {{ $selectedProduct->category->name ?? 'Oferta' }}
                                    </span>
                                </div>
                                                        ...
                                                        @if($variant->is_active && $variant->stock_quantity > 0)
                                                            <button type="button" 
                                                                ...
                                                                @if($variant->price_adjustment != 0)
                                                                    <span class="ml-1 font-medium {{ isset($selectedVariants[$variantName]) && $selectedVariants[$variantName] == $variant->id ? 'text-red-100' : 'text-red-600' }}">
                                                                        {{ $variant->price_adjustment > 0 ? '+' : '' }}{{ number_format($variant->price_adjustment, 0, ',', '.') }} Kz
                                                                    </span>
                                                                @endif
                                                                <span class="ml-1 {{ isset($selectedVariants[$variantName]) && $selectedVariants[$variantName] == $variant->id ? 'text-red-100' : 'text-gray-500' }}">({{ $variant->stock_quantity }})</span>
                                                            </button>
                                                        @else
                                ...
                                <div class="flex space-x-3">
                                    <button wire:click="addToCart({{ $selectedProduct->id }})" 
                                            class="flex-1 bg-gradient-to-r from-red-500 to-pink-500 text-white py-3 px-4 rounded-lg font-semibold hover:from-red-600 hover:to-pink-600 transition-colors">
